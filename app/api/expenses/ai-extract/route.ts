...
המשתמש הוא עוסק פטור ישראלי. תפקידך: לקרוא את הקבלה (תמונה או PDF) ולחלץ את הנתונים המבניים בדיוק 100%.
...
- "amount": הסכום הסופי לתשלום שנכתב בקבלה (בש"ח), כולל אגורות. **חוקים מחייבים:**
  1. חפש את השורה שמסומנת "סה"כ לתשלום" / "סך הכל" / "סה"כ סופי" / "Total" / "לחיוב" / "Grand Total" — **רק** את המספר שכתוב שם.
  2. אל תיקח: סכום מע"מ לבד · סכום לפני מע"מ (subtotal) · מחיר של פריט בודד · אחוז הנחה.
  3. כלול את האגורות בדיוק כפי שנכתב (לדוגמה 149.90, לא 150).
  4. אם הקבלה מציגה כמה סכומים — תמיד קח את המספר הגדול ביותר במקום של "סה"כ לתשלום".
  5. אם הקבלה מציגה רק סכום אחד (בלי פירוט) — קח אותו.
  6. החזר מספר עשרוני עם 2 ספרות אחרי הנקודה. דוגמאות תקינות: 149.90, 75.50, 200.00, 1234.56.
  7. אל תעגל לעולם.
  8. אם אינך בטוח ב-100% מהי המספר הנכון — החזר null. עדיף ריק על שגוי.
...
- החזר רק JSON תקין, בלי הסבר ובלי markdown.
- אם אינך רואה קבלה בכלל (תמונה ריקה / משהו לא רלוונטי), החזר את כל השדות null.`;
  ...
  try {
    ...
    const message = await client.messages.create({
      ...
      max_tokens: 500,
      // temperature: 0 = deterministic output. Same input → identical output.
      // Critical for amount accuracy — eliminates the small "creative
      // wobble" that could make the model pick a different number on
      // different runs of the same receipt.
      temperature: 0,
      system: systemPrompt,
      messages: [
        {
